...
- Renovación: republicar toda rebanada con más de 20 días desde su última publicación, registro local en MMKV (`ckey → última publicación`).
- Migración de servidor: 1 sola, `supabase/006_compaction_por_rebanada.sql`, agrega `ckey text` nullable a `envelopes`, cambia el trigger de compactación para incluir `and ckey is not distinct from new.ckey`, reemplaza el índice de compactación. Cero borrado/reescritura de filas existentes. **Se aplica ANTES que el build de la app** (orden obligatorio, ADR-007 §5).
...
- **Renovación (§3.4 de ADR-007):** test de que una rebanada no tocada en 20+ días se republica sola en el siguiente ciclo de sync.
